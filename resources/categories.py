...
class CategoryByName(Resource): #'/api/categories/<str:category_name>'
    ...
    def get(self, category_name): #return note ids 
        ...
        try:
            ...
            cat_id = cat.id
            note_ids = db.session.query(NoteCategory).filter(NoteCategory.category_id == cat_id).all()
            output = []
    ...
    def delete(self, category_name): #delete a specific category
        ...
        try:
            ...
            rows = db.session.query(NoteCategory).filter(NoteCategory.category_id == cat_id).all()
            count = len(rows)
            for r in rows:
                db.session.delete(r)
            db.session.commit()
